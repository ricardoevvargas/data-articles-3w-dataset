...
The 3W Dataset is a collection of Multivariate Time Series (MTS) [[9]](#9)[[10]](#10), referred to as instances, which have been labeled by experts from Petrobras and its partners. The name "3W" was chosen because the dataset comprises instances derived from ***3*** different sources (real, simulated, and hand-drawn) which contains undesirable events that occur in oil ***W***ells. Each instance may either represent 100% of the data associated with normal operating conditions or contain data partially related to a specific type of undesirable event. The core idea behind the dataset is to enable the learning (modeling) of temporal patterns or signatures across multiple variables, distinguishing between normal conditions and various types of undesirable events.

The main features of the 3W Dataset are as follows. Its real instances correspond to data collected directly from real industrial environments. Characteristics such as frozen variables, missing variables, and outliers are intentionally left untreated. Thos approach aims to to encourage and enable the development of methodologies and digital products capable of dealing with real-world challenges. By preserving these typical characteristics, the dataset maintains a high level of quality and realism. Simulated instances have been added because some types of undesirable events are rare in real-world operations. Hand-drawn instances have been added to address events that are not only rare but also challenging to simulate. Experts can use their understanding of variable behaviors during such events to manually create these instances to mirror real-world conditions as closely as possible.

Due to its features, the 3W Dataset can also serve as a foundational resource for training basic models in Transfer Learning frameworks [[11]](#11). These frameworks rely on a large, generalized dataset as a reference or starting point for training models to address different but related challenges, particularly when an adequate training dataset for the target task is unavailable. This approach is widely used in Deep Learning [[12]](#12), especially in scenarios requiring large amounts of training data.

The 3W Dataset is managed using Semantic Versioning [[13]](#13), with its initial release designated as version 1.0.0.

Since its initial release, the 3W Dataset has been explored by several people who make up the 3W Community [[14]](#14), including independent professionals and representatives of research institutions, startups, companies, and oil operators from different countries. 
...
In 2022, Petrobras officially launched the 3W Project as the inaugural pilot of the Open Lab Module within the Connections for Innovation Program [[16]](#16). The purpose of this module is to encourage open and collaborative project development on the Internet, particularly through the GitHub platform [[17]](#17). Since then, the 3W Dataset has been maintained and developed in its dedicated corporate Git repository [[15]](#15) on GitHub.

As part of the 3W Project, the same Git repository also hosts the 3W Toolkit, a software package developed in Python 3 [[18]](#18). The toolkit is designed to facilitate and encourage the exploration of the 3W Dataset, as well as the development and comparison of different methodological approaches.
...
Since 2022, the 3W Dataset has been evolved under Petrobras' leadership, with its current publicly available version designated as 2.0.0.

This data article describes the 3W Dataset 2.0.0 and summarizes the advancements  incorporated since version 1.0.0. The detailed description is intended to encourage and support the established 3W Community and also new users to improve previously published results and develop innovative methodologies, digital products and services. These advancements aim  to enable the early detection of undesirable events in oil wells, allowing for timely corrective or mitigating actions.
...
The nomenclature used in this work is derived from the definition of MTS provided in the previous subsection. The specific terms and their descriptions are presented in Table 1.
...
Each type of instance has been defined based on the origin of its data.  Real instances were sourced from various Petrobras' Plant Information Management Systems (PIMS) [[19]](#19), specifically  different AVEVA PI System [[20]](#20) environments. Simulated instances were generated using OLGA [[21]](#21), a dynamic multiphase flow simulator widely adopted by oil companies worldwide. Hand-drawn instances were created by Petrobras experts and extracted using a digital tool developed exclusively for this purpose.
...
All instances were generated with observations recorded every 1 second, resulting in a fixed sampling frequency of 1 Hz. This sampling refers to data retrieval through the PI System, with some observations being interpolated to ensure a consistent time series.
...
- 1: Abrupt Increase of BSW: A sudden rise in Basic Sediment and Water ratio, potentially leading to flow assurance issues, reduced oil production, and operational challenges.
- 2 Spurious Closure of DHSV: An unexpected closure of Downhole Safety Valve (DHSV), risking production losses.
- 3 Severe Slugging: Periodic and intense flow instability that can damage equipment and disrupt operations.
- 4 Flow Instability: Non-periodic flow disturbances that, if unaddressed, may escalate into severe slugging.
- 5: Rapid Productivity Loss: A sudden decrease in well productivity, often driven by changes in system properties.
- 6: Quick Restriction in PCK: A rapid and significant restriction in the production choke valve, typically caused by operational challenges.
- 7: Scaling in PCK: Formation of inorganic deposits in the production choke valve, reducing oil and gas production.
- 8: Hydrate in Production Line: Formation of crystalline compounds (hydrates) that can block pipelines, leading to significant production losses and high unblocking costs.
- 9: Hydrate in Service Line: Similar to event 8, but occurring in the service line, leading to distinct signature patterns in operational data.

For further details on these events, please refer to Vargas et al. [[8]](#8).

The codes associated with the state labels are detailed in Table 5. These codes can be associated with any observation from any instance, but they are not associated with an instance as a whole. If the well's operating condition is unknown at any given time, the respective observation is assigned the state label Unknown (code = NaN).
...
Each operational status is defined based on the configuration of the valves and the nature of recent or ongoing operations. In summary, the statuses are as follows:
...
The particularities of the method developed for generating real data are listed below.

* Data acquisition:
    1. Data is sourced from Petrobras' PIMS [[19]](#19), specifically from various AVEVA PI System [[20]](#20) environments;
    1. Linear interpolation provided by the AVEVA PI System is applied to simulate a fixed acquisition frequency of 1 Hz;
    1. No treatment is performed for frozen variables or missing valuesç
...
* Data labeling:
    ...
    1. Labelers use a Petrobras-developed Web Tool to label and export real data to the 3W Dataset;
    ...
    1. Fig. 2 illustrates the labeling process and Fig. 3 provides an example of a labeled real instance.
...
* Data generation:
    1. A matrix of simulations was planned with OLGA [[21]](#21), gradually varying the main parameters of the considered scenario;
...
1. A simplified simulation strategy was employed. For example, hydrates were simulated using a valve with a linear closing percentage;
...
* Data generation:
    ...
    1. Time series were automatically digitized by scanning graphs printed on paper;
    1. Time series are perfectly periodic;
    1. No frozen variables or missing values are present;
    1. Variables are represented with standardized measurement units.
* Data labeling:
    1. Labels were derived from expert markings on the graphs printed on paper, including transient periods.
...
1. The process depends on the expert's manual dexterity.
...
The 3W dataset's main statistics related to the inherent difficulties of real data are presented in Table 7.

**Table 7**<br>The 3W dataset's main statistics related to the inherent difficulties of real data.
...
In the root of the directory containing the dataset, there is a file called dataset.ini, which specifies the properties of the 3W Dataset 2.0.0. The proposal is for all users to concentrate their searches for these properties in this file.

The data itself is organized into subdirectories dedicated to each type of event. The name of each directory corresponds to the code associated with each event type (see Table 6).

Each instance is stored in its own Apache Parquet file [[27]](#27), or simply Parquet file. Parquet is an open source, column-oriented data file format designed for efficient data storage and retrieval. It supports high-performance compression and encoding schemes, allowing it to handle complex data in bulk. It is also supported in many programming languages and analytics tools.

The logic for formulating file names depends on the type of instance.

- **Real Instances:**  
  File names follow the format: WELL-[incremental id]_[timestamp of oldest observation].parquet.  
  Example: WELL-00014_20170917140000.parquet.  
  Each real well is associated with a unique ID, regardless of the event type (subdirectory). As each real well can give rise to one or multiple instances, the [timestamp of oldest observation] ensures unique identification.

- **Simulated Instances:**  
  File names follow the format: SIMULATED_[incremental id].parquet.  
  Example: SIMULATED_00072.parquet.  
...
- **Hand-Drawn Instances:**  
  File names follow the format: DRAWN_[incremental id].parquet.  
  Example: DRAWN_00007.parquet.  
...
All Parquet files are created with the Pyarrow engine [[28]](#28) and the Brotli compression [[29]](#29). These choices provide a good compromise between compression ratio and read performance.
...
Several carefully incorporated features in the methods described in the Methods Section ensure the high-technical quality of the 3W Dataset 2.0.0. The main features are as follows:

* Real instances: maintenance of real data characteristics, labeling by experts, and validation by expert committee;
...
This data article describes the 3W Dataset 2.0.0, the latest publicly available version, and summarizes its evolution from version 1.0.0, as previously detailed by Vargas et al. [[8]](#8).

The 3W Dataset 2.0.0 was generated with Python 3.10 [[18]](#18) code, using mainly resources provided by the Pandas 1.5 [[30]](#30) and Pyarrow 19.0 [[28]](#28) packages. 

Designed for cross-platform compatibility, the 3W Dataset 2.0.0 can be analyzed using virtually any programming language—not limited to Python. However, regardless of the language, the Apache Parquet files [[27]](#27) should be accessed using the Pyarrow engine [[28]](#28) alongside Brotli compression [[29]](#29).
...
The main updates introduced in version 2.0.0, compared to version 1.0.0, are summarized below. More details can be found in the release notes available in the 3W Project repository [[15]](#15).
...
* Petróleo Brasileiro S.A. (Petrobras), Rio de Janeiro, Brazil
    * Cláudio Benevenuto de Campos Lima, Ricardo Emanuel Vaz Vargas, Jean Carlos Dias de Araújo, Mateus de Araujo Fernandes & Afrânio José de Melo Junior
